...
std::string TranslateUpXMLForATV::TranslateManualTaskSchedulerQuery(const  XMLTask* task,std::vector<sTaskInfo> &taskinfovec )
{
	...
	PROPMANAGER::instance()->GetTaskDeviceList(std::string("AutoRecord"), ATV,devicedlist);	//只获取录像通道
...
{
	std::string retXml=GetXmlHeader(task,"TVChannelCheckReport");//标准的返回头xml
...
{
	std::string retXml=GetXmlHeader(task,"DeviceInfoCheckResp");//标准的返回头xml
	...
	paser.SetAttrNode("Manufactures",string("金石"),InfoNode);
...
{
	std::string retXml=GetXmlHeader(task,"ServerStatusSet");//标准的返回头xml
	return retXml;
}
