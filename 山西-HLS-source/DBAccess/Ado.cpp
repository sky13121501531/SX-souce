...
{
    string msg = string("执行SQL语句: ") + lpstrExec;
		...
		case VT_DATE:
			{
                ...
                //if(strDateFormat.c_str().length()<1)
...
BOOL CADORecordset::Execute(CADOCommand* pAdoCommand)
{
    ...
    string msg = "执行SQL语句: " + pAdoCommand->GetText();
//    CAppLog::GetInstance()->WriteLog(OTHER,LOG_EVENT_DEBUG,msg.c_str());
...
AdoDbOper::AdoDbOper()
{
    ...
    {
        string msg = "无法定位数据库,请确认数据库"+Fdatabase+"是否存在？";
    ...
    {
        CAppLog::GetInstance()->WriteLog(OTHER, LOG_EVENT_ERROR, "数据库只读,无法执行程序！");
        delete fileInfo;
        getchar();
        exit(0);
    }
